        ...
        <div className="grid md:grid-cols-2 gap-6 lg:gap-8">
          {useCases.map((useCase, index) => (
            ...
            >
              ...
              {/* Label preview image */}
              <div className="px-4 sm:px-8 pb-6 sm:pb-8 [perspective:1000px]">
                <div className="bg-white rounded-xl p-2 sm:p-3 border border-border/50 shadow-sm group-hover:shadow-md transition-all duration-700 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)]">
                  <img 
                    src={useCase.image} 
                    alt={`${useCase.title} example`}
                    className="w-full max-w-[200px] sm:max-w-[280px] mx-auto h-auto rounded-lg [backface-visibility:hidden]"
                  />
                </div>
              </div>
            </div>
